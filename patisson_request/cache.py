...
class RedisAsyncCache(BaseAsyncTTLCache):
    ...
    async def set(self, key: str, value: Encodable, time: Optional[Seconds | timedelta] = None) -> None:
        ...
        """
        try:
            await self.redis.set(name=key, value=value, ex=time if time else self.default_cache_lifetime)
...
class MemcachedAsyncCache(BaseAsyncTTLCache):
    ...
    Attributes:
        ...
        memcached_client (pymemcache.client.base.Client): The Memcached client instance used
            for communication.
    """
