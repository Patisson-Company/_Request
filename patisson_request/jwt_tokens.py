...
It provides base classes and specific implementations for both access and refresh tokens, as well
as utilities for serializing and validating token-related data. The module supports client and service roles,
along with specific permissions for each.
...
- **BaseTokenPayload**: A generic base class for token payloads containing common fields such as `type`,
    `iss`, `sub`, `exp`, and `iat`.
- **BaseAccessTokenPayload**: A subclass of `BaseTokenPayload` that extends it with additional fields like
    `bearer` and `role`, which represent the bearer type (client or service) and the associated
    role with permissions.
- **ClientAccessTokenPayload**: A subclass of `BaseAccessTokenPayload` specifically for client access tokens.
    It uses client-specific roles and permissions.
- **ServiceAccessTokenPayload**: A subclass of `BaseAccessTokenPayload` for service access tokens, associated
    with service-specific roles and permissions.
- **RefreshTokenPayload**: A subclass of `BaseTokenPayload` that represents refresh token payloads.
- **mask_token**: A utility function to mask part of the token string, leaving only the last few
    characters visible.
...
- `BaseTokenPayload`: Base class for token payloads that includes common attributes like `type`, `iss`,
    `sub`, `exp`, and `iat`.
- `BaseAccessTokenPayload`: Extends `BaseTokenPayload`, adding fields for `bearer` and `role`
    (which represent the token bearer and the associated role).
- `ClientAccessTokenPayload`: A subclass of `BaseAccessTokenPayload` used for client access tokens.
    It handles client-specific role serialization and validation.
- `ServiceAccessTokenPayload`: A subclass of `BaseAccessTokenPayload` used for service access tokens.
    It handles service-specific role serialization and validation.
- `RefreshTokenPayload`: A subclass of `BaseTokenPayload` for refresh token payloads, with the same structure
    but specifically for refresh tokens.
- `mask_token`: A function that masks the characters of the token except for the last `visible_chars` number
    of characters, providing a way to securely display tokens.

Usage:
    - To create access token payloads for clients or services, instantiate `ClientAccessTokenPayload`
        or `ServiceAccessTokenPayload`, respectively.
