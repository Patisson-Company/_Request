"""
This module defines roles with associated permissions for different entities within the system, including service and client roles. 

The roles are designed to be immutable and provide strict validation of permissions. Each role corresponds to a set of permissions that govern the access and functionality for a particular entity. 

Key Concepts:
- **ServicePermissions**: Defines a set of permissions related to the service layer, such as media access, user authentication, user registration, and forum access.
- **ClientPermissions**: Defines a set of permissions for client operations, such as creating libraries, banning users, and using chat.
- **Role**: A class that encapsulates a role name and its associated permissions. It is generic and can represent either `ServicePermissions` or `ClientPermissions`.
- **_EntityRoles**: A base class for immutable role enumerations, ensuring that roles cannot be modified after creation. Provides a lookup method for retrieving role objects by name.
- **_ServiceRole and _ClientRole**: Concrete implementations of `_EntityRoles` for service and client roles, respectively. These classes define a set of predefined roles with their associated permissions.
- **Role Lookup**: Roles can be accessed via the class (e.g., `ServiceRole._TEST`) or by calling the class with the role name (e.g., `ServiceRole('SERVES_USERS')`).
...
Classes:
    ...
    - `_EntityRoles`: A generic base class for immutable role enumerations, ensuring role immutability and providing lookup functionality.
    - `_ServiceRole`: A concrete class for service roles, defining common roles such as `_TEST`, `MINIMUM`, and `MEDIA_ACCESS`.
    - `_ClientRole`: A concrete class for client roles, defining common roles such as `_TEST`, `MEMBER`, `ADMIN`, and `OWNER`.

Usage:
    - To define new roles or to retrieve existing roles, you can instantiate or call the appropriate role enumeration class (`ServiceRole` or `ClientRole`).
    - Roles can be retrieved by calling the role class with the role name, e.g., `ServiceRole('MEDIA_ACCESS')`.
    - The permissions associated with each role can be accessed through the `permissions` property of the `Role` class.
...
class Role(Generic[Permissions]):
    ...
    Args:
        ...
        permissions (Permissions): The set of permissions associated with the role. 
                                   This is a generic type that can be either `ServicePermissions` or `ClientPermissions`.
