...
def build_query(
    ...
    fields: Sequence[GraphqlField | NestedGraphqlFields],
) -> str:
    """
    ...
    return (
        f'{type} {{ {name}{'(' + query_args + ')' if query_args != "" else ""}'
        f"{{ {build_query_fields(fields)} }} }}"
    )  # noqa: E501


def build_query_fields(fields: Sequence[GraphqlField | NestedGraphqlFields]) -> str:
    """
    Construct the fields portion of a GraphQL query, handling both simple and nested fields.
...
def format_strings(sequence: Sequence[str]) -> str:
    """
    Format a sequence of strings into a GraphQL array format.
