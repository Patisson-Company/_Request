...
and JWT validation errors. Custom exceptions are defined for handling specific cases, such as invalid JWTs
or service authorization errors.

Key Concepts:
- **ErrorCode**: Enum that defines a set of error codes used throughout the application. Each code represents
    a specific error message, such as invalid parameters, bad credentials, or JWT-related issues
    (e.g., JWT expiration or invalid sub).
- **ErrorSchema**: A Pydantic model that represents the error response structure. It includes the error code
    and an optional extra message for additional details.
...
- **InvalidJWT**: A custom exception class that is raised when there is an issue with a JWT, such as
    invalid JWT, expiration, or mismatched sub. The exception contains an `ErrorSchema` that describes
    the error.
- **UnauthorizedServiceError**: A custom exception class for unauthorized service access.

Classes and Enums:
- `ErrorCode`: Enum that defines specific error codes and their corresponding messages, covering a wide range
    of authentication, authorization, and validation errors.
- `ErrorSchema`: A Pydantic model that encapsulates error information, including the error code and an
...
- `InvalidJWT`: Exception raised for invalid JWTs, containing an `ErrorSchema` to provide detailed
    error information.
- `UnauthorizedServiceError`: Exception raised when a service is not authorized to access
    a particular resource.

Usage:
    - To raise an error, instantiate the appropriate exception class, such as `InvalidJWT`
        or `UnauthorizedServiceError`, and provide any necessary details (e.g., `ErrorSchema` for JWT errors).
    - Use the `ErrorSchema` model to structure error responses for HTTP exceptions, returning
        the corresponding error code and optional extra message.
