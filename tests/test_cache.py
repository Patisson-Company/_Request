...
from patisson_request.cache import (
    BaseAsyncTTLCache,
    MemcachedAsyncCache,
    RedisAsyncCache,
)
...
async def test_redis_set_get_cache():
    redis_cache = RedisAsyncCache(service=Service._TEST, logger=logger)
    await base_set_get_cache(redis_cache)


@pytest.mark.asyncio
async def test_memcached_set_get_cache():
    memcached_cache = MemcachedAsyncCache(service=Service._TEST, logger=logger)
