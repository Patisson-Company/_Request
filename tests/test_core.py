...
from patisson_request.service_responses import ErrorBodyResponse_4xx, ErrorBodyResponse_5xx, TokensSetResponse
...
def mock_auth(httpx_mock: HTTPXMock) -> HTTPXMock:
    def request_callback(request: httpx.Request) -> httpx.Response:
        ...
        if (
            request.url == "http://localhost/authentication/api/v1/service/jwt/create"
            ...
            request.url == "http://localhost/authentication/api/v1/service/jwt/update"
            and request.method == "POST"
        ):
            body = json.loads(request.content.decode("utf-8"))
            ...
            return httpx.Response(
                status_code=400,
                content=json.dumps({"detail": [ErrorSchema(error=ErrorCode.ACCESS_ERROR).model_dump()]}),
            )

        elif request.url == "http://localhost/authentication/500":
            assert request.headers.get("Authorization") == f"Bearer {TEST_ACCESS_TOKEN}"
            return httpx.Response(status_code=500, content="test error")
...
async def test_200_request_method(SelfService: SelfAsyncService, mock_auth: HTTPXMock):
    response = await SelfService.post_request(
        *-AuthenticationRoute.api.v1.service.jwt.update(refresh_token=TEST_REFRESH_TOKEN)
    )
...
async def test_400_request_method(SelfService: SelfAsyncService, mock_auth: HTTPXMock):
    response = await SelfService.get_request(
        ...
        path="400",
        response_type=TokensSetResponse,  # false call
    )
    assert isinstance(response, Response)
